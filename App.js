...
export default function App() {
  ...
  async function enviarUsername() {
    ...
    try {
      const pendingKey = 'nexa_pending_internal_transfer';
      const destinationUsername =
        recipientUser.username || normalizeUsername(username);
      ...
      try {
        ...
        if (storedPending) {
          ...
          if (
            parsedPending.fromUserId === user.id &&
            parsedPending.toUsername === destinationUsername &&
            Number(parsedPending.amountUsdc) === Number(amountToSend)
          ) {
            pendingRequest = parsedPending;
          }
        }
      } catch (e) {}
      ...
      show(data);
      if (data.success) {
        await AsyncStorage.removeItem('nexa_pending_internal_transfer');
        await carregarDados();
